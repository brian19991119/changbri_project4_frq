...
fork1(void)
{
  ...
  pid = fork();
      41:	e8 04 0d 00 00       	call   d4a <fork>
  if(pid == -1)
    ...
    wait();
      4f:	e8 06 0d 00 00       	call   d5a <wait>
  while(getcmd(buf, sizeof(buf)) >= 0){
  ...
  pid = fork();
      7c:	e8 c9 0c 00 00       	call   d4a <fork>
  if(pid == -1)
    ...
    panic("fork");
      ...
      96:	50                   	push   %eax
      97:	e8 de 0c 00 00       	call   d7a <close>
      break;
      9c:	83 c4 10             	add    $0x10,%esp
      9f:	eb b3                	jmp    54 <main+0x54>
  exit();
      a1:	e8 ac 0c 00 00       	call   d52 <exit>
      runcmd(parsecmd(buf));
...
00000100 <getcmd>:
{
  ...
  memset(buf, 0, nbuf);
     ...
     11e:	6a 00                	push   $0x0
     120:	53                   	push   %ebx
     121:	e8 8a 0a 00 00       	call   bb0 <memset>
  gets(buf, nbuf);
     126:	58                   	pop    %eax
     127:	5a                   	pop    %edx
     128:	56                   	push   %esi
     129:	53                   	push   %ebx
     12a:	e8 e1 0a 00 00       	call   c10 <gets>
  if(buf[0] == 0) // EOF
...
00000150 <panic>:
{
  ...
  exit();
     165:	e8 e8 0b 00 00       	call   d52 <exit>
...
00000170 <runcmd>:
{
  ...
  switch(cmd->type){
    ...
    exec(ecmd->argv[0], ecmd->argv);
     ...
     19c:	52                   	push   %edx
     19d:	50                   	push   %eax
     19e:	e8 e7 0b 00 00       	call   d8a <exec>
    ...
    exit();
     1b8:	e8 95 0b 00 00       	call   d52 <exit>
  pid = fork();
     1bd:	e8 88 0b 00 00       	call   d4a <fork>
  if(pid == -1)
    ...
    close(rcmd->fd);
     ...
     1e0:	e8 95 0b 00 00       	call   d7a <close>
    if(open(rcmd->file, rcmd->mode) < 0){
     1e5:	59                   	pop    %ecx
     1e6:	58                   	pop    %eax
     1e7:	ff 73 10             	pushl  0x10(%ebx)
     1ea:	ff 73 08             	pushl  0x8(%ebx)
     1ed:	e8 a0 0b 00 00       	call   d92 <open>
  ...
  pid = fork();
     237:	e8 0e 0b 00 00       	call   d4a <fork>
  if(pid == -1)
    ...
    close(p[0]);
     ...
     24b:	e8 2a 0b 00 00       	call   d7a <close>
    close(p[1]);
     250:	58                   	pop    %eax
     251:	ff 75 f4             	pushl  -0xc(%ebp)
     254:	e8 21 0b 00 00       	call   d7a <close>
    wait();
     259:	e8 fc 0a 00 00       	call   d5a <wait>
    wait();
     25e:	e8 f7 0a 00 00       	call   d5a <wait>
    break;
     263:	83 c4 10             	add    $0x10,%esp
     266:	e9 4d ff ff ff       	jmp    1b8 <runcmd+0x48>
  pid = fork();
     26b:	e8 da 0a 00 00       	call   d4a <fork>
  if(pid == -1)
    ...
    wait();
     27d:	e8 d8 0a 00 00       	call   d5a <wait>
    runcmd(lcmd->right);
    ...
    panic("fork");
     ...
     2aa:	6a 00                	push   $0x0
     2ac:	e8 c9 0a 00 00       	call   d7a <close>
      dup(p[0]);
     2b1:	5a                   	pop    %edx
     2b2:	ff 75 f0             	pushl  -0x10(%ebp)
     2b5:	e8 10 0b 00 00       	call   dca <dup>
      close(p[0]);
     2ba:	59                   	pop    %ecx
     2bb:	ff 75 f0             	pushl  -0x10(%ebp)
     2be:	e8 b7 0a 00 00       	call   d7a <close>
      close(p[1]);
     2c3:	58                   	pop    %eax
     2c4:	ff 75 f4             	pushl  -0xc(%ebp)
     2c7:	e8 ae 0a 00 00       	call   d7a <close>
      runcmd(pcmd->right);
     2cc:	58                   	pop    %eax
     ...
     2e5:	6a 01                	push   $0x1
     2e7:	e8 8e 0a 00 00       	call   d7a <close>
      dup(p[1]);
     2ec:	58                   	pop    %eax
     2ed:	ff 75 f4             	pushl  -0xc(%ebp)
     2f0:	e8 d5 0a 00 00       	call   dca <dup>
      close(p[0]);
     2f5:	58                   	pop    %eax
     2f6:	ff 75 f0             	pushl  -0x10(%ebp)
     2f9:	e8 7c 0a 00 00       	call   d7a <close>
      close(p[1]);
     2fe:	58                   	pop    %eax
     2ff:	ff 75 f4             	pushl  -0xc(%ebp)
     302:	e8 73 0a 00 00       	call   d7a <close>
      runcmd(pcmd->left);
     307:	58                   	pop    %eax
...
00000310 <fork1>:
{
  ...
  pid = fork();
     316:	e8 2f 0a 00 00       	call   d4a <fork>
  if(pid == -1)
...
execcmd(void)
{
  ...
  memset(cmd, 0, sizeof(*cmd));
     ...
     345:	6a 00                	push   $0x0
     347:	50                   	push   %eax
     348:	e8 63 08 00 00       	call   bb0 <memset>
  cmd->type = EXEC;
...
pipecmd(struct cmd *left, struct cmd *right)
{
  ...
  memset(cmd, 0, sizeof(*cmd));
     ...
     3c5:	6a 00                	push   $0x0
     3c7:	50                   	push   %eax
     3c8:	e8 e3 07 00 00       	call   bb0 <memset>
  cmd->type = PIPE;
  cmd->left = left;
     ...
     469:	8b 45 08             	mov    0x8(%ebp),%eax
{
  ...
  while(s < es && strchr(whitespace, *s))
     ...
     493:	e8 38 07 00 00       	call   bd0 <strchr>
  ...
  while(s < es && strchr(whitespace, *s))
     ...
     4e3:	e8 e8 06 00 00       	call   bd0 <strchr>
  ...
  *ps = s;
  ...
  switch(*s){
    ...
    while(s < es && !strchr(whitespace, *s) && !strchr(symbols, *s))
     ...
     53c:	e8 8f 06 00 00       	call   bd0 <strchr>
    ...
    while(s < es && !strchr(whitespace, *s) && !strchr(symbols, *s))
     ...
     55b:	e8 70 06 00 00       	call   bd0 <strchr>
...
peek(char **ps, char *es, char *toks)
{
  ...
  while(s < es && strchr(whitespace, *s))
     ...
     603:	e8 c8 05 00 00       	call   bd0 <strchr>
  ...
  *ps = s;
  ...
  return *s && strchr(toks, *s);
     ...
     621:	e8 aa 05 00 00       	call   bd0 <strchr>
...
00000a50 <parsecmd>:
{
  ...
  es = s + strlen(s);
     ...
     a5b:	53                   	push   %ebx
     a5c:	e8 1f 01 00 00       	call   b80 <strlen>
  cmd = parseline(&s, es);
     a61:	59                   	pop    %ecx
  es = s + strlen(s);
...
void unlock(struct spinlock * lk) {
     ...
     af8:	f0 87 02             	lock xchg %eax,(%edx)
  xchg(&lk->locked, 0);
}
     afb:	5d                   	pop    %ebp
     afc:	c3                   	ret    
     ...
     e19:	85 d2                	test   %edx,%edx
{
  ...
  write(fd, &c, 1);
     ...
     e80:	e8 ed fe ff ff       	call   d72 <write>
...
printf(int fd, const char *fmt, ...)
{
  ...
  write(fd, &c, 1);
     ...
     ef0:	e8 7d fe ff ff       	call   d72 <write>
  ...
  write(fd, &c, 1);
     ...
     f51:	e8 1c fe ff ff       	call   d72 <write>
  ...
  write(fd, &c, 1);
     f6a:	e8 03 fe ff ff       	call   d72 <write>
  for(i = 0; fmt[i]; i++){
  ...
  write(fd, &c, 1);
    ...
    1003:	57                   	push   %edi
    1004:	53                   	push   %ebx
    1005:	e8 68 fd ff ff       	call   d72 <write>
        while(*s != 0){
  ...
  write(fd, &c, 1);
    ...
    1057:	e8 16 fd ff ff       	call   d72 <write>
        ap++;
...
malloc(uint nbytes)
{
  ...
  p = sbrk(nu * sizeof(Header));
    ...
    116e:	56                   	push   %esi
    116f:	e8 66 fc ff ff       	call   dda <sbrk>
  if(p == (char*)-1)
